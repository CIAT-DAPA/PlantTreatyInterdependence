...
create table SGSV_CLEAN as (select accession_number as original_id, 
...
-- uniques
create table SGSV_CLEAN_uniques as select * from (
    select * 
    from SGSV_CLEAN
    group by id, genus
)j;
...
create table CROP_ORIGIN_GENUS_SGSV as 
(select c.crop, g.orig_country, count(*) count
FROM SGSV_CLEAN_uniques g
...
create table CROP_ORIGIN_SPECIES_SGSV  as 
(select c.crop, g.orig_country, count(*) count
FROM SGSV_CLEAN_uniques g
...
create table CROP_INSTITUTION_GENUS_SGSV  as 
...
create table CROP_INSTITUTION_SPECIES_SGSV  as 
(select c.crop,  g.institution_country, count(*) count
FROM SGSV_CLEAN_uniques g
left join CIAT_crop_taxon c on (g.species=c.taxon)
where c.rank="species"
...
create table CROP_GENUS_SGSV  as 
...
create table CROP_SPECIES_SGSV  as 
(select c.crop,  count(*) count
FROM SGSV_CLEAN_uniques g
left join CIAT_crop_taxon c on (g.species=c.taxon)
where c.rank="species"
