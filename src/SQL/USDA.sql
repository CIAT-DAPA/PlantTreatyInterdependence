...
select c.crop, sum(u.`count samples distributed`) from CIAT_crop_taxon c
...
select c.crop, u.country_code, u.year, sum(u.`count samples distributed`) from CIAT_crop_taxon c
left join USDA_demand u on c.taxon = u.Species
where c.rank="species"
and u.country_code is not null
group by c.crop, u.country_code, u.year;