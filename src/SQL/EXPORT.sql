...
ifnull(cpg.count,0) as upov_genus_varietal_release,
ifnull(cps.count,0) as upov_species_varietal_release
...
ifnull(sum(genus_accessions_sgsv) / NULLIF(sum(genus_count_institution_supply)+sum(genus_accessions_sgsv), 0) , 0)
as genus_accessions_sgsv,
ifnull(sum(genus_accessions_sgsv) / NULLIF(sum(species_count_institution_supply)+sum(genus_accessions_sgsv), 0) , 0)
